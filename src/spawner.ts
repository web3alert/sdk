...
export type ChangedCallback<T> = (prev: T, next: T) => boolean;
...
export class Spawner<T, I extends Destructible> {
  private _changed: ChangedCallback<T>;
  private _callback: SpawnerCallback<T, I>;
  private _mutex: Mutex;
  private _items: Map<string, { value: T, item: I }>;
  ...
  private async _spawnItem(key: string, value: T): Promise<void> {
    try {
      const item = await this._callback(key, value);
      this._items.set(key, { value, item });
    } catch (err) {
      throw new Web3alertError('spawner spawn item error', {
        cause: err,
        details: { key },
      });
    }
  ...
  private async _destroyItem(key: string): Promise<void> {
    try {
      ...
      throw new Web3alertError('spawner destroy item error', {
        cause: err,
        details: { key },
      });
    }
  ...
  private async _maybeRespawnItem(key: string, value: T): Promise<void> {
    ...
    if (entry) {
      if (!this._changed(entry.value, value)) {
        return;
      }
