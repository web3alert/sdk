...
import { Web3alertError, deserializeError } from './errors';
...
import { type SpawnerCallback, type ChangedCallback, Spawner } from './spawner';
...
export class Core {
  ...
  constructor(options: CoreOptions) {
    ...
    this.sliceSpawner = async (name, slice, changed, callback) => {
      const spawner = new SliceSpawner({ core: this, name, slice, changed, callback });
      await spawner.init();
  ...
  public async nothrow(callback: Callback, details?: Record<string, unknown>): Promise<void> {
    try {
      ...
      this.error(new Web3alertError('uncaught exception', { cause: err, details }));
    }
  }
    ...
    const timer = new Timer({
      ...
      callback: async now => {
        try {
          ...
          this.uncaughtException(new Web3alertError('timer callback failed', {
            cause: err,
            details: { timer: name },
          }));
        }
    ...
    const scheduler = new Scheduler({
      callback: async (now, schedule) => {
        try {
          ...
          this.uncaughtException(new Web3alertError('scheduler callback failed', {
            cause: err,
